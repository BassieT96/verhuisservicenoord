...
export async function generateMetadata() {
  ...
  if (!page) {
    return createPageMetadata({
      ...
      description: "Vul de verhuisdiensten pagina in Sanity.",
      path: `/${slug}`,
    });
  }
...
export default async function VerhuisdienstenPage() {
  const [page, settings] = await Promise.all([getCmsContentPage(slug), getCmsSiteSettings()]);
  if (!page || !settings) return <SanityEmptyState pageName="Verhuisdiensten" />;
